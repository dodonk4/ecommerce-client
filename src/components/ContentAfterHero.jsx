import React from 'react'

function ContentAfterHero() {
  return (
    <section id='ContentAfterHero' className='contentAfterHeroBox'>
        <div className='contentAfterHero'>
             <div className='promo1 promo'>
              <img className="promo1Image" src='images/gtx1660.png'></img>
              <div className='promoTextAfterHero'>
                <div>COMPONENTES</div>
              </div>
            </div>
            <div className='promo2 promo'>
              <img className="promo2Image" src='images/flip.png'></img>
              <div className='promoTextAfterHero'>
                <div>CELULARES</div>
              </div>
            </div>
            <div className='promo3 promo'>
              <img className='promo3Image' src='images/jblflip5.png'></img>
              <div className='promoTextAfterHero'>
                <div>PARLANTES</div>
              </div>
              
            </div>
            <div className='promo4 promo'>
              <img className="promo4Image" src='images/noblexastro39.png'></img>
              <div className='promoTextAfterHero'>
                <div>TELEVISORES</div>
              </div>
            </div>
            <div className='promo5 promo'>
              <img className="promo5Image" src='images/g413.png'></img>
              <div className='promoTextAfterHero'>
                <div>PERIFERICOS</div>
              </div>
            </div>
            <div className='promo6 promo'>
            <img className="promo6Image" src='images/dellinspiron.png'></img>
              <div className='promoTextAfterHero'>
                <div>COMPUTADORAS</div>
              </div>
            </div>
        </div>
        


    </section>
    
  )
}

export default ContentAfterHero